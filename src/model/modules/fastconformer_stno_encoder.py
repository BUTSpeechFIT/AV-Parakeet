...
class ConformerEncoderSTNO(NeuralModule, StreamingEncoder, Exportable, AccessMixin):
    """
...
class ConformerEncoderSTNOAdapter(ConformerEncoderSTNO, adapter_mixins.AdapterModuleMixin):
    """This class inherits from ConformerEncoderSTNO and wraps the adapter mixin class."""
...
class ConformerMultiLayerFeatureExtractor(NeuralModule, Exportable, AccessMixin):
    """
    A wrapper module that extracts features from multiple layers of a ConformerEncoderSTNO,
    ...
    def forward(
        ...
        # Chunk of code adapted from ConformerEncoderSTNO.forward_internal()
        total_registry = {}
        ...
        for layer_idx in self.layer_idx_list:
            try:
                ...
                raise RuntimeError(
                    ...
                    "Check the layer index and the number of ConformerEncoderSTNO layers."
                )
...
if adapter_mixins.get_registered_adapter(ConformerEncoderSTNO) is None:
    adapter_mixins.register_adapter(base_class=ConformerEncoderSTNO, adapter_class=ConformerEncoderSTNOAdapter)
