...
from .asr_model import EncDecRNNTModelSTNOAV
from src.metrics.meeteval_mt_wer import MeetevalMTWER
from src.data.lhotse_dataset_setup import get_av_to_text_and_stno_lhotse_dataset


class EncDecRNNTBPEModelSTNOAV(EncDecRNNTModelSTNOAV, ASRBPEMixin):
    ...
    def change_vocabulary(
        ...
        del self.joint
        self.joint = EncDecRNNTBPEModelSTNOAV.from_config_dict(new_joint_config)

        decoder_config = self.decoder.to_config_dict()
        new_decoder_config = copy.deepcopy(decoder_config)
        new_decoder_config.vocab_size = len(vocabulary)
        del self.decoder
        self.decoder = EncDecRNNTBPEModelSTNOAV.from_config_dict(new_decoder_config)
    ...
    def _setup_dataloader_from_config(self, config: Optional[Dict], val: bool = False):
        ...
        if config.get("use_lhotse"):
            ...
            raise NotImplementedError("We require the usage of Lhotse dataset. NeMo manifests are not implemented!")
        ...
        def seed_worker(worker_id):
            ...
            # Torch seeding can also be forced here, but it is usually unnecessary.
